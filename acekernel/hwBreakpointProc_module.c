...
static atomic64_t g_hook_pc1;
static atomic64_t g_hook_pc2;
static atomic64_t g_hook_pc3;
static atomic64_t g_hook_pc4;
static atomic64_t g_hook_pc5;
static atomic64_t g_hook_pc6;

static atomic64_t hook_addr1;
static atomic64_t hook_addr2;
static atomic64_t hook_addr3;
static atomic64_t hook_addr4;
static atomic64_t hook_addr5;
static atomic64_t hook_addr6;
...
static void hwbp_handler(struct perf_event *bp,
	...
	struct pt_regs *regs) {
	citerator iter;
	printk_debug(KERN_INFO "hw_breakpoint HIT!!!!! bp:%px, pc:%px\n", bp, regs->pc);
...
static void hwbp_handler1(struct perf_event *bp,
	struct perf_sample_data *data,
	struct pt_regs *regs) {
	uint64_t hook_pc1;
	printk_debug(KERN_INFO "hw_breakpoint HIT!!!!! bp:%px, pc:%px\n", bp, regs->pc);
	hook_pc1 = atomic64_read(&g_hook_pc1);
	if (hook_pc1 != 0) {
		regs->pc = hook_pc1;
		return;
	}
...
static void hwbp_handler2(struct perf_event *bp,
	struct perf_sample_data *data,
	struct pt_regs *regs) {

	uint64_t hook_pc2;
	printk_debug(KERN_INFO "hw_breakpoint HIT!!!!! bp:%px, pc:%px\n", bp, regs->pc);
	hook_pc2 = atomic64_read(&g_hook_pc2);
	if (hook_pc2 != 0) {
		regs->pc = hook_pc2;
		return;
	}
...
static void hwbp_handler3(struct perf_event *bp,
	struct perf_sample_data *data,
	struct pt_regs *regs) {

	uint64_t hook_pc3;
	printk_debug(KERN_INFO "hw_breakpoint HIT!!!!! bp:%px, pc:%px\n", bp, regs->pc);
	hook_pc3 = atomic64_read(&g_hook_pc3);
	if (hook_pc3 != 0) {
		regs->pc = hook_pc3;
		return;
	}
...
static void hwbp_handler4(struct perf_event *bp,
	struct perf_sample_data *data,
	struct pt_regs *regs) {
		
	uint64_t hook_pc4;
	printk_debug(KERN_INFO "hw_breakpoint HIT!!!!! bp:%px, pc:%px\n", bp, regs->pc);
	hook_pc4 = atomic64_read(&g_hook_pc4);
	if (hook_pc4 != 0) {
		regs->pc = hook_pc4;
		return;
	}
...
static void hwbp_handler5(struct perf_event *bp,
	struct perf_sample_data *data,
	struct pt_regs *regs) {

	uint64_t hook_pc5;
	printk_debug(KERN_INFO "hw_breakpoint HIT!!!!! bp:%px, pc:%px\n", bp, regs->pc);
	hook_pc5 = atomic64_read(&g_hook_pc5);
	if (hook_pc5 != 0) {
		regs->pc = hook_pc5;
		return;
	}
...
static void hwbp_handler6(struct perf_event *bp,
	struct perf_sample_data *data,
	struct pt_regs *regs) {
			
	uint64_t hook_pc6;
	printk_debug(KERN_INFO "hw_breakpoint HIT!!!!! bp:%px, pc:%px\n", bp, regs->pc);
	hook_pc6 = atomic64_read(&g_hook_pc6);
	if (hook_pc6 != 0) {
		regs->pc = hook_pc6;
		return;
	}
...
static ssize_t OnCmdInstProcessHwbp(struct ioctl_request *hdr, char __user* buf) {
	...
	uint64_t aaddr1, aaddr2, aaddr3, aaddr4, aaddr5, aaddr6;
	...
	aaddr1 = atomic64_read(&hook_addr1);
	aaddr2 = atomic64_read(&hook_addr2);
	aaddr3 = atomic64_read(&hook_addr3);
	aaddr4 = atomic64_read(&hook_addr4);
	aaddr5 = atomic64_read(&hook_addr5);
	aaddr6 = atomic64_read(&hook_addr6);

	if (proc_virt_addr == aaddr1)
		hwbp_handle_info.sample_hbp = x_register_user_hw_breakpoint(&hwbp_handle_info.original_attr, hwbp_handler1, NULL, task);
	else if (proc_virt_addr == aaddr2)
		hwbp_handle_info.sample_hbp = x_register_user_hw_breakpoint(&hwbp_handle_info.original_attr, hwbp_handler2, NULL, task);
	else if (proc_virt_addr == aaddr3)						
		hwbp_handle_info.sample_hbp = x_register_user_hw_breakpoint(&hwbp_handle_info.original_attr, hwbp_handler3, NULL, task);
	else if (proc_virt_addr == aaddr4)
		hwbp_handle_info.sample_hbp = x_register_user_hw_breakpoint(&hwbp_handle_info.original_attr, hwbp_handler4, NULL, task);	
	else if (proc_virt_addr == aaddr5)	
		hwbp_handle_info.sample_hbp = x_register_user_hw_breakpoint(&hwbp_handle_info.original_attr, hwbp_handler5, NULL, task);			
	else if (proc_virt_addr == aaddr6)
		hwbp_handle_info.sample_hbp = x_register_user_hw_breakpoint(&hwbp_handle_info.original_attr, hwbp_handler6, NULL, task);
	else
...
static ssize_t OnCmdSetHookPc(struct ioctl_request *hdr, char __user* buf) {
	...
	uint64_t id = hdr->param2;
	uint64_t addr = hdr->param3;
	if (id == 0) {
		atomic64_set(&hook_addr1, addr);
		atomic64_set(&g_hook_pc1, pc);
	} else if (id == 1) {
		atomic64_set(&hook_addr2, addr);
		atomic64_set(&g_hook_pc2, pc);
	} else if (id == 2) {
		atomic64_set(&hook_addr3, addr);
		atomic64_set(&g_hook_pc3, pc);
	} else if (id == 3) {
		atomic64_set(&hook_addr4, addr);
		atomic64_set(&g_hook_pc4, pc);
	} else if (id == 4) {	
		atomic64_set(&hook_addr5, addr);
		atomic64_set(&g_hook_pc5, pc);
	} else if (id == 5) {
		atomic64_set(&hook_addr6, addr);
		atomic64_set(&g_hook_pc6, pc);
	}
